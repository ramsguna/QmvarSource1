...
Public Class Analysis_Cash_Tracking1
    ...
    Protected Sub btnSend_Click(sender As Object, e As ImageClickEventArgs) Handles btnSend.Click
        ...
        If Warranty2 <> "IW" Then
            If ChkCash.Checked = False And ChkCredit.Checked = False Then
                Call showMsg("paymentにチェックしてください。 ", "")
                Exit Sub
            End If
        End If
        ...
        If Warranty2 = "deposit" Then
            ...
            If errFlg = 1 Then
                Call showMsg("From T_Reserve, we failed to acquire amount of OPEN processing", "")
                Exit Sub
            End If
            ...
            If errFlg = 1 Then
                Call showMsg("I failed to acquire the deposit total amount from cash_track.", "")
                Exit Sub
            End If
            ...
            End If
            '*************************************
            'Added Mohan on 2019.01.22
            'Need to Find total collected amount
            '登録済のdeposit金額を取得
            Dim CollectedAmt As Decimal
            strSQL = "SELECT CONVERT(decimal(13,2),SUM(claim)) AS CollectedAmt FROM dbo.cash_track WHERE DELFG = 0 AND location = '" & shipCode & "' "
            strSQL &= "AND payment='Cash' and Warranty in ('OOW','Other') AND FALSE = '0' AND LEFT(CONVERT(VARCHAR, invoice_date, 111), 10) = '" & dtNow.ToShortDateString.Substring(0, 5) & setMon2 & "/" & setDay2 & "'"
            Dim DT_cash_track1 As DataTable = DBCommon.ExecuteGetDT(strSQL, errFlg)
            If errFlg = 1 Then
                Call showMsg("Collected amount query errror。", "")
                Exit Sub
            End If
            If DT_cash_track1 IsNot Nothing Then
                If DT_cash_track1.Rows(0)("CollectedAmt") IsNot DBNull.Value Then
                    CollectedAmt = DT_cash_track1.Rows(0)("CollectedAmt")
                End If
            End If
            '*******************************************************
            ...
            TotalAmt = openTotal + CollectedAmt

            '''Mohan modified on 2019.01.22
            '''
            'if Branch is service center then minimum petty cash keep with them 
            'Dim shipCode As String = Session("ship_code")
            ...
            If Not (_MoneyReserveModel.BRType = "DR") Then 'Not neccessary for Data Recovery
                Dim ReserveAmt As Integer = 0, DiffAmt As Decimal = 0.00
                ReserveAmt = Integer.Parse(ConfigurationManager.AppSettings("ReserveAmt"))
                DiffAmt = TotalAmt - work
                If DiffAmt < ReserveAmt Then
                    Call showMsg("The petty cash cann't be below of " & ReserveAmt, "")
                    Exit Sub
                End If
            End If


            'Comment by Mohan on 20190122
            ' If work > openTotal Then
            If work > TotalAmt Then
